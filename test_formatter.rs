...
//! e ajuste o arquivo .sqruff para customizar a formatação.
...
fn main() {
    ...
    // Debug: ver o que sqruff produz sem pós-processamento
    use sqruff_lib::core::config::FluffConfig;
    use sqruff_lib::core::linter::core::Linter;
    let config = FluffConfig::from_root(None, false, None).unwrap_or_else(|_| FluffConfig::default());
    let linter = Linter::new(config, None, None, false);
    let result = linter.lint_string(query6, None, true);
    println!("DEBUG - Sqruff raw output:\n{}\n", result.fix_string());
    ...
    // println!("\nPara ajustar a formatação, edite o arquivo .sqruff na raiz do projeto.");
}
